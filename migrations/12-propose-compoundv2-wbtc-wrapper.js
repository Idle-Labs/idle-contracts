...
module.exports = async (deployer, network, accounts) => {
  ...
  await deployer.deploy(IdleCompoundV2, addresses.WBTC.live, addresses.cWBTCV2.live, {from: creator});
  ...
  await compoundV2Wrapper.setIdleToken(addresses.idleWBTCV4, {from: creator});
  ...
  for (const idleTokenName in idleTokens) {
    ...
    const params = [
      ...
      [tokens, wrappers, [20000, 20000, 10000, 40000, 10000], true]
    ];
  ...
  let propName = 'add compoundv2 wrapper';
  let proposal = {
    targets: targets,
    values: values,
