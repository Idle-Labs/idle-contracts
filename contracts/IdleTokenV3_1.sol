...
contract IdleTokenV3_1 is ERC20, ERC20Detailed, ReentrancyGuard, Ownable, Pausable, IIdleTokenV3, GST2Consumer {
  ...
  function setMaxUnlentPerc(uint256 _perc)
    external onlyOwner {
      require(_perc <= 100000, "Unlent % too high");
      maxUnlentPerc = _perc;
  }
  ...
  function withdrawNotUsedTokens(address _token, uint256 _amount)
    external onlyOwner {
      require(_token != token, 'Cannot withdraw underlying used');

      for (uint256 i = 0; i < allAvailableTokens.length; i++) {
        require(_token != allAvailableTokens[i], 'Cannot withdraw interest-bearing tokens used');
      }
  ...
  function _rebalance(uint256, bool _skipWholeRebalance)
    ...
    returns (bool) {
      ...
      emit Rebalance(msg.sender, totalRedeemd);
