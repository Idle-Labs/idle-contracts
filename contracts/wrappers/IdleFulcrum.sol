...
contract IdleFulcrum is ILendingProtocol {
  ...
  function mint()
    ...
    returns (uint256 iTokens) {
      ...
      iTokens = iERC20Fulcrum(iToken).mint(msg.sender, balance);
  }
  ...
  function redeem(address _account)
    ...
    // Funds needs to be sended here before calling this
      tokens = iERC20Fulcrum(iToken).burn(_account, IERC20(token).balanceOf(address(this))));
  }
}
