...
contract('IdleRebalancer', function ([_, creator, nonOwner, someone, foo]) {
  beforeEach(async function () {
    ...
    this.IdleRebalancerWithFulcrumMock = await IdleRebalancer.new(
      this.cDAIMock.address,
      this.iDAIMock.address,
      this.cDAIWrapper.address,
  ...
  // TODO not sure on how to test it correctly
  // it('calcRebalanceAmounts when fulcrum utilization rate > 90%', async function () {
  //   const newDAIAmount = BNify('100000000').mul(this.one); // 100.000.000 DAI
  //
  //   // set Params for cDAIMock
  //   const val = [];
  ...
  //   valFulcrum[1] = BNify('806330399262842122707083'), // totalAssetBorrow;
  //   valFulcrum[2] = BNify('838941079486105304319308'), // totalAssetSupply();
  //   valFulcrum[3] = BNify('90000000000000000000'), // spreadMultiplier();
  //
  //   // with rebalanceCalcTest 100000000 I got in last iteration
  //   // { fulcrumAmount: '2037271.7592491972475',
  //   //   compoundAmount: '97962728.2407508027525',
  //   //   fulcNewRate: '1.13905027677805058072',
  //   //   compNewRate: '1.170591471925300489' }
  //   // 97962728.2407508027525 DAI in compound, 2037271.7592491972475 DAI fulcrum ####################
  //
  //   // set mock data in cDAIMock
  //   await this.iDAIMock.setParams(valFulcrum);
  //   // set next rate with utilization rate > 90%
  //   await this.iDAIWrapperMock._setNextSupplyRate(BNify(this.one).mul(BNify('2'))); // 2%
  //   const res = await this.IdleRebalancerWithFulcrumMock.calcRebalanceAmounts(newDAIAmount, { from: creator });
  //
  //   res.tokenAddresses[0].should.be.equal(this.cDAIMock.address);
  //   res.tokenAddresses[1].should.be.equal(this.iDAIMock.address);
  //   res.amounts[0].should.be.bignumber.equal(BNify('99697793203834249560105414')); // 99704548 DAI compound
